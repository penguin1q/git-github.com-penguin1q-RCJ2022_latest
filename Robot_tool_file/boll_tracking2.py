...
class Robot_moves:
    ...
    """[summary]
        ...
        out: 出力[x,y]->ndarray
    """
    def movepos(self, now_pos, tar_pos, dt, p_gain = 5.8, d_gain = 0.95, range=64):
        now_pos = np.array(now_pos)
        tar_pos = np.array(tar_pos)
        #P = np.sqrt((tar_pos - now_pos)/233.82)
        Px = (tar_pos[0] - now_pos[0])
        Py = (tar_pos[1] - now_pos[1])
        if Px**2 + Py**2 < range:
            Px = 0
            Py = 0
        """x_P = np.sign(Px)*abs(Px/233.82)**(1/2.5)
        y_P = np.sign(Py)*abs(Py/233.82)**(1/2.5)
        P = np.array([x_P, y_P])"""
        ...
        D = (P - self.preP) / dt
        self.preP = P
        """if P[0]**2 + P[1]**2 < range:
            return np.array([0,0])
        else:"""
        out = P * p_gain + D * d_gain
        if 0<(abs(out[0]) + abs(out[1])) < 140:
            max_val = math.sqrt(out[0]**2 + out[1]**2)
            outx = int(out[0] / max_val * 140)
            outy = int(out[1] / max_val * 140) 
            out = [outx, outy]
        """if (abs(out[0]) + abs(out[1])) > 190:
            #max_val = max(abs(out[0]), abs(out[1]))
            max_val = math.sqrt(out[0]**2 + out[1]**2)
            outx = int(out[0] / max_val * 190)
            outy = int(out[1] / max_val * 190)
            out = [outx, outy]
        if 0<(abs(out[0]) + abs(out[1])) < 170:
            max_val = math.sqrt(out[0]**2 + out[1]**2)
            outx = int(out[0] / max_val * 170)
            outy = int(out[1] / max_val * 170)
            if 0<abs(out[0])<50 or 0<abs(out[1])<50:
                outx = int(out[0] / max_val * 180)
                outy = int(out[1] / max_val * 180)
            #if 0<abs(out[1])<50:
            #    outx = int(out[0] / max_val * 180)
            out = [outx, outy]"""
        print(f"{int(out[0]):>3} {int(out[1]):>3} {now_pos[0]:3} {now_pos[1]:3}", end=" ")
